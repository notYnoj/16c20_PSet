...
def step_RK4(thisIVP, dt, un, tn):
    ...
    k2 = thisIVP.evalf( [un[i] + k1[i] * dt/2.0 for i in range(len(un))], tn + dt/2)
    k3 = thisIVP.evalf([un[i] + k2[i] * dt/2.0 for i in range(len(un))], tn + dt/2)
    k4 = thisIVP.evalf( [un[i] + k3[i] * dt for i in range(len(un))] , tn + dt)
    un1 = [(un[i] + dt/6 * (k1[i] + 2 * k2[i] + 2 * k3[i] + k4[i])) for i in range(len(un))]
...
def solve(thisIVP, dt, method):
    ...
    time = tI
    while time <= tF:
        #diff eq is defined as ti, ui, dt, and current time method then solves this for us
        nextU = method(thisIVP, dt, u[-1], time)
        time+=dt
        u.append(nextU)
        t.append(time)
    return (t, u)
